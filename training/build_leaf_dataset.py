"""
RU: Скрипт для подготовки бинарного датасета leaf / not_leaf
EN: Script to prepare binary dataset leaf / not_leaf
"""
...
PLANTVILLAGE_DIR = BASE_DIR / "filtered_dataset"  / "val"   # ← сюда ты кладёшь уже отфильтрованный PlantVillage (после prepare_data)
LEAF_DATASET_DIR = BASE_DIR / "leaf_detector_dataset"
LEAF_DIR = LEAF_DATASET_DIR / "leaf"
NOT_LEAF_DIR = LEAF_DATASET_DIR / "not_leaf"
...
# 🧹 Чистка папок
# ================================
if LEAF_DATASET_DIR.exists():
    shutil.rmtree(LEAF_DATASET_DIR)
LEAF_DIR.mkdir(parents=True, exist_ok=True)
NOT_LEAF_DIR.mkdir(parents=True, exist_ok=True)

# ================================
# 📥 1. Копируем все листья (leaf)
# ================================
count_leaf = 0
for class_dir in PLANTVILLAGE_DIR.iterdir():
    if class_dir.is_dir():
        for img_path in class_dir.glob("*"):
            if img_path.suffix.lower() in [".jpg", ".jpeg", ".png"]:
                shutil.copy(img_path, LEAF_DIR / img_path.name)
                count_leaf += 1

print(f"✅ Скопировано {count_leaf} изображений листьев")

# 📥 2. Скачиваем not_leaf из FiftyOne
print(f"📥 Скачиваем {MAX_SAMPLES} картинок not_leaf из Open Images V7...")
dataset = foz.load_zoo_dataset(
    "open-images-v7",
    split="validation",
    classes=NOT_LEAF_CLASSES,
...
for sample in dataset:
    ...
    shutil.copy(sample.filepath, NOT_LEAF_DIR / f"not_leaf_{count_not_leaf}{ext}")
    count_not_leaf += 1
...
# ================================
print("\n📦 Датасет готов:")
print(f"  leaf: {len(list(LEAF_DIR.glob('*')))}")
print(f"  not_leaf: {len(list(NOT_LEAF_DIR.glob('*')))}")
print(f"  Путь: {LEAF_DATASET_DIR}")
